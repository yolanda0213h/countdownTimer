...
class FirstViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        timeMove = Timer.scheduledTimer(withTimeInterval: 1, repeats: true, block: { (_) in
            ...
            let interval = yearDate!.timeIntervalSince(nowDate) + 1
            
            let sky10 = ["甲","乙","丙","丁","戊","己","庚","辛","壬","癸"]
            let groud12 = ["子","丑","寅","卯","辰","巳","午","未","申","酉","戌","亥"]
            var i = 0
            if Int(interval/86400) > 60 {
                i = Int(interval/86400)%60
                if i < 9 {
                    chineseDate = sky10[9-i] + groud12[9-i] + chineseDate
                }
                else {
                    let a = 9-(Int(interval/86400)%60 - 9)%10
                    let b = 11-(Int(interval/86400)%60 - 9)%12
                    chineseDate = sky10[a] + groud12[b] + chineseDate
                }
            }
            else{
             if Int(interval/86400) < 9 {
                chineseDate = sky10[9-i] + groud12[9-i] + chineseDate
             }
             else if Int(interval/86400) >= 9 {
                let a = 9-(Int(interval/86400) - 9)%10
                let b = 11-(Int(interval/86400) - 9)%12
                chineseDate = sky10[a] + groud12[b] + chineseDate
             }
             
            }
            if (Int(interval/86400) - 9) == 2 {
                chineseMonth = "丑月"
            }
            if (Int(interval/86400) - 9) == 1 {
                chineseMonth = "寅月"
            }
            let chineseTime = chineseYear + "-" + chineseDate + "-" + chineseMonth + " \n" + chineseHour + "時" + chineseMins
